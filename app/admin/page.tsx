...
export default function AdminFinal(){
  ...
  // FIXED - Ab khulega
  const sendOTP=async()=>{ const {error}=await supabase.auth.signInWithOtp({email:LOCKED_EMAIL}); if(error){alert(error.message);return} setOtpSent(true); alert("OTP भेजा "+LOCKED_EMAIL+" pe - Spam bhi dekho") }
  const doReset=async()=>{ if(!otp||!newPass){alert("OTP + New Pass likho");return} const {error}=await supabase.auth.verifyOtp({email:LOCKED_EMAIL,token:otp,type:"email"}); if(error){alert("OTP Galat");return} localStorage.setItem("safa_pass",newPass); alert("Done! New: "+newPass); setShowForgot(false); setOtpSent(false) }
