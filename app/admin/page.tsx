    ...
    <div className="min-h-screen bg-[#f8f9f6] flex text-[13px]">
      <div className="flex-1">
        <div className="bg-white border-b px-4 py-3 flex justify-between">
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search..." className="border rounded-lg px-4 py-2"/>
          <button onClick={()=>{setEditingProduct(null);setForm({name:'',price:'',original_price:'',sale_price:'',category:'',image_url:'',image_urls:[],affiliate_link:'',is_best_seller:false,is_featured:false,is_active:true,display_theme:'default'});setShowAddForm(true);}} className="bg-[#c49a4b] px-4 py-2 rounded-lg font-bold">+ Add Product</button>
        </div>
        <div className="p-4">
          <div className="bg-white rounded-xl p-4 border">
            <p className="font-bold mb-3">Recent Products - {filtered.length} - PROMISE 1+2+3 LOCK</p>
            <div className="grid grid-cols-5 gap-3">
              {filtered.slice(0,10).map((p:any)=>(
                <div key={p.id} className="border rounded-xl p-2.5">
                  ...
                  <p className="text-[11px] mt-2">{p.name}</p>
