...
export default function AdminPage(){
  ...
  // ORIGINAL ME YE THE - AB SIRF 3 ADD KIYE: original_price, sale_price, image_urls
  ...
  const handleFetchDaraz=async()=>{
    ...
    try{
      ...
      if(d.success && d.price){
        setForm(f=>({...f,name:d.name||f.name,price:String(d.price),sale_price:String(d.price),original_price:String(Number(d.price)+500),image_url:d.image||f.image_url,image_urls:d.image?[d.image,...f.image_urls].slice(0,4):f.image_urls}));
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                  {filtered.slice(0,10).map((p:any)=>(
                    <div key={p.id} className="border rounded-xl p-2.5 relative">
                      ...
                      <p className="text-[12px] font-bold"><span className="line-through text-[10px] text-gray-400 mr-1">Rs.{p.original_price||Number(p.price)+500}</span> Rs. {p.sale_price||p.price} PKR</p>
          ...
          <div className="bg-white rounded-[16px] w-full max-w-[500px] p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input required type="number" placeholder="Sale Price - 1499" value={form.sale_price} onChange={e=>setForm({...form,sale_price:e.target.value,price:e.target.value})} className="w-full border rounded-lg px-3 py-2.5 text-[13px] bg-green-50 font-bold border-green-300"/>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input type="hidden" value={form.price}/>
                <select required value={form.category} onChange={e=>setForm({...form,category:e.target.value})} className="w-full border rounded-lg px-3 py-2.5 text-[13px] bg-yellow-50 font-bold col-span-2"><option value="">Category - Public Control</option>{categoriesList.map((c:any)=><option key={c.slug} value={c.name}>{c.name}</option>)}{categories.map((c:string)=><option key={c} value={c}>{c}</option>)}<option value="Kitchen">Kitchen</option><option value="Bartan">Bartan</option><option value="Storage">Storage</option></select>
              </div>
              <div className="p-3 border rounded-lg bg-gray-50">
                ...
                {imageUploading&&<p className="text-[10px] text-blue-600">Uploading to Supabase...</p>}
                <div className="grid grid-cols-4 gap-2 mt-2">{form.image_urls.map((u:string,i:number)=><img key={i} src={u} className="w-full h-14 rounded-lg border object-cover"/> )}</div>
