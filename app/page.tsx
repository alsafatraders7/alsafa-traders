          ...
          {filtered.map((p: any) => (
            <div key={p.id} className="bg-white border rounded-[18px] overflow-hidden shadow-sm">
              <div onClick={() => { setSelected(p); loadReviews(p.id) }} className="cursor-pointer">
                <img src={p.image_url || "https://via.placeholder.com/400"} className="h-[190px] w-full object-cover" alt="" />
                <div className="p-3">
                ...
                <div className="mt-3 space-y-2 max-h-40 overflow-auto">
                  {reviews.map((r: any) => (
                    <div key={r.id} className="bg-gray-50 p-2 rounded-lg">
                      <p className="font-bold text-xs">{r.customer_name} — {"★".repeat(r.rating)}</p>
