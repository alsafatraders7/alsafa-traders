...
async function getDarazData(url: string) {
  try {
    ...
    const discount = discountMatch? parseInt(discountMatch[1]) : (original > price? Math.round(((original-price)/original)*100) : 0);

    return { price, original, discount, isFlash };
  } catch {
    return null;
  }
}
...
export async function GET() {
  try {
    ...
    for (const prod of products || []) {
      ...
      if (data) {
        const newPrice = Math.round(data.price * 1.1); // Tumhara 10% Profit
    ...
    return NextResponse.json({
      ...
      updated,
      message: `LIVE DONE! ${updated} Products Updated - Discount, Flash Sale, Price LIVE + Affiliate?cc`,
